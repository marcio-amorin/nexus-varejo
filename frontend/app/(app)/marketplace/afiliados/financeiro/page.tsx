...
const hoje = new Date()
const FORM_INIT = {
  plataforma:'ML_AFILIADOS', titulo_produto:'',
  data_venda: hoje.toISOString().slice(0,10),
  valor_venda:'', comissao_pct:'', comissao_valor:'', status:'PENDENTE'
}
...
export default function FinanceiroAfiliados() {
  ...
  async function registrar(e:any) {
    e.preventDefault()
    try {
      await fetch(`${API}/afiliados/comissoes`, {
    ...
    <div className="pg">
      ...
      <div className="pg-stats flex gap-2 items-center flex-wrap">
        ...
        {['','PENDENTE','APROVADO','PAGO'].map(s => (
          <button key={s} onClick={() => setFiltroSt(s)}
            ...
            style={{
              background: filtroSt===s ? (s ? STATUS_COR[s]?.bg : '#f97316') : 'var(--card2)',
              color:      filtroSt===s ? (s ? STATUS_COR[s]?.cor : '#fff')   : 'var(--muted)',
              border:     filtroSt===s ? 'none' : '1px solid var(--border)',
            }}>
            {s || 'Todos'}
          </button>
        ))}
          ...
          <table className="tbl">
            ...
            <tbody>
              {comissoes.map((c:any, i:number) => {
                ...
                return (
                  <tr key={i}>
                    <td className="font-medium text-white" style={{ maxWidth:160 }}>
                      <p className="truncate">{c.titulo_produto || '—'}</p>
                    </td>
                      ...
                      <div className="flex gap-1">
                        {c.status==='PENDENTE' && (
                          <button onClick={() => atualizarStatus(c.id,'APROVADO')}
                            ...
                            style={{ background:'rgba(59,130,246,0.2)', color:'#3b82f6' }}>Aprovar</button>
                        )}
                        {c.status==='APROVADO' && (
                          <button onClick={() => atualizarStatus(c.id,'PAGO')}
                            className="text-[10px] px-2 py-1 rounded-lg font-bold"
                            style={{ background:'rgba(34,197,94,0.2)', color:'#22c55e' }}>Pago</button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {showForm && (
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background:'rgba(0,0,0,0.7)' }}>
          <form onSubmit={registrar} className="rounded-2xl overflow-hidden w-full max-w-md mx-4"
            style={{ background:'var(--card)', border:'1px solid #f97316' }}>
            ...
            <div className="p-4 space-y-2.5">
              {[
                ...
                { label:'Valor da Venda',   key:'valor_venda',    type:'number', ph:'0.00' },
